...
while restart_count < 5 and has_errors:
    ...
    try:
        if not 2666926487380378949 in succeeded_hashes:  # avoid duplicate inserts
            instance = user1 = User(username="user_one", email="user1@example.com", password="hashed_pw1")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(2666926487380378949)
    except Exception as e:
        has_errors = True
    ...
    try:
        if not 3440625116992876137 in succeeded_hashes:  # avoid duplicate inserts
            instance = profile1 = Profile(first_name="John", last_name="Doe", bio="Bio of John Doe", user_id=user1.id)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(3440625116992876137)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 8118637899426331877 in succeeded_hashes:  # avoid duplicate inserts
            instance = post1 = Post(title="First Post", content="This is the first post content.", user_id=user1.id)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(8118637899426331877)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 7144996797356788495 in succeeded_hashes:  # avoid duplicate inserts
            instance = comment1 = Comment(content="Nice post!", post_id=post1.id, user_id=user1.id)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(7144996797356788495)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not -1558631466443691898 in succeeded_hashes:  # avoid duplicate inserts
            instance = like1 = Like(post_id=post1.id, user_id=user1.id)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-1558631466443691898)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 1567824390741461815 in succeeded_hashes:  # avoid duplicate inserts
            instance = follow1 = Follow(follower_id=user1.id, followee_id=another_user.id)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(1567824390741461815)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
